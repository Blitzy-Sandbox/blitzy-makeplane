 ...
 *    server URL (or `undefined` on failure). In `apps/web`, the consumer-supplied
 *    implementation wraps the apps/api v2 presigned-POST asset upload contract:
 *      1. `POST assets/v2/workspaces/<slug>/` (or the project-scoped equivalent
 *         `…/projects/<project_id>/`) returns `{ upload_data, asset_id, asset_url }`
 *         — `upload_data` is the presigned-POST envelope (URL + signed form fields)
 *         generated by `S3Storage.generate_presigned_post` against MinIO/S3.
 *      2. The browser uploads the file bytes directly to object storage using the
 *         presigned envelope — this hook never touches the binary upload path.
 *      3. `PATCH assets/v2/workspaces/<slug>/<asset_id>/` marks the asset
 *         `is_uploaded = True` and queues `get_asset_object_metadata.delay()` for
 *         async metadata extraction.
 *    This hook consumes only the resolved `asset_url` returned by `editorCommand`. See
 *    `apps/api/plane/app/views/asset/v2.py` (`WorkspaceFileAssetEndpoint`,
 *    `ProjectAssetEndpoint`) and the parallel `TFileHandler.upload` documentation in
