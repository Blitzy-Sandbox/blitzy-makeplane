...
"""Google OAuth endpoints for the Plane app authentication flow.

This module exposes the two endpoints that implement Google OAuth 2.0
Authorization Code flow on the app surface:

  * :class:`GoogleOauthInitiateEndpoint` -- ``GET /auth/google/``
      Generates a fresh ``state`` (``uuid.uuid4().hex``), stores it in
      ``request.session["state"]``, stores the app host and any caller-
      supplied ``next_path`` in the session, and 302-redirects to Google's
      authorization URL produced by :class:`GoogleOAuthProvider`.

  * :class:`GoogleCallbackEndpoint` -- ``GET /auth/google/callback/``
      Receives the ``code`` and ``state`` Google sends back, validates the
      returned ``state`` against the session-stored value (OAuth CSRF
      defense), exchanges the ``code`` for a Plane :class:`User` via
      :meth:`GoogleOAuthProvider.authenticate`, invokes
...
``AUTHENTICATION_ERROR_CODES["GOOGLE_OAUTH_PROVIDER_ERROR"]`` and a 302
...
:data:`AUTHENTICATION_ERROR_CODES` keys raised:
  ...
  * ``GOOGLE_OAUTH_PROVIDER_ERROR`` -- state mismatch, missing ``code``,
...
Architectural notes (per AAP section 0.2.2):
  ...
  * No Celery / RabbitMQ enqueue from this module; provider integration
    handoff is synchronous through :class:`GoogleOAuthProvider`.
"""
...
class GoogleOauthInitiateEndpoint(View):
    ...
    HTTP method / URL:
        ``GET /auth/google/`` (route ``google-initiate`` declared in
    ...
    Response:
        * HTTP 302 redirect to the Google authorization URL (success).
    ...
    Side effects:
        ...
        * Constructs :class:`GoogleOAuthProvider` and calls
          :meth:`GoogleOAuthProvider.get_auth_url`.

    Error codes:
        ``INSTANCE_NOT_CONFIGURED``, ``GOOGLE_OAUTH_PROVIDER_ERROR``.
    """

    def get(self, request):
        """Stash session state, generate the OAuth ``state`` token, and redirect to Google."""
...
class GoogleCallbackEndpoint(View):
    """Validate the OAuth callback and complete Google sign-in.

    HTTP method / URL:
        ``GET /auth/google/callback/`` (route ``google-callback`` declared
        in ``apps/api/plane/authentication/urls.py``).

    Inheritance:
        :class:`django.views.View` -- no ``permission_classes`` declared.
        Effectively anonymous-accessible because Google redirects the
        user here before they are authenticated in Plane.

    Query parameters (provided by Google's redirect):
    ...
    Side effects:
        * Validates ``state`` against ``request.session.get("state",
          "")``; mismatch raises ``GOOGLE_OAUTH_PROVIDER_ERROR``.
        * Validates presence of ``code``; missing raises
          ``GOOGLE_OAUTH_PROVIDER_ERROR``.
        * Constructs :class:`GoogleOAuthProvider` with
          ``callback=post_user_auth_workflow`` and calls
          :meth:`GoogleOAuthProvider.authenticate` -- exchanges the code
    ...
    Error codes:
        ``GOOGLE_OAUTH_PROVIDER_ERROR`` (covers state mismatch, missing
