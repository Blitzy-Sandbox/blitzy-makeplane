...
"""Serializer for the ``Module`` model in the ``plane.space`` public API.

Defines :class:`ModuleBaseSerializer`, the full ``Module`` payload embedded
inside :class:`plane.space.serializer.issue.IssueModuleDetailSerializer`
...
class ModuleBaseSerializer(BaseSerializer):
    """Full ``Module`` payload with ownership and audit fields write-protected.

    Wraps :class:`plane.db.models.Module` with ``fields = "__all__"`` so every
    ...
    * ``created_at`` and ``updated_at`` -- timestamp bookkeeping is managed
    ...
    Consumed via ``module_detail`` on
    :class:`plane.space.serializer.issue.IssueModuleDetailSerializer`, which
    embeds this payload as a nested projection when serializing module
    membership inside issue detail responses on published deploy boards.
    """

    class Meta:
        """DRF metadata: full ``Module`` payload with ownership/audit fields read-only."""
