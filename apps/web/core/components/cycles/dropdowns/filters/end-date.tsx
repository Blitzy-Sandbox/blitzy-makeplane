...
/**
 * Cycle due-date (end-date) filter section.
 *
 * Preset due-date filter checkbox group sourced from
 ...
 * a re-render. Rendered inside a `FilterHeader` titled "Due date" with a
 * count suffix when one or more values are applied. Architecturally mirrors
 * the `FilterStartDate` sibling; only the user-facing titles differ.
 *
 * Props:
 *   - `appliedFilters: string[] | null` (required) — currently selected
 *     due-date filter tokens; `null` is treated as "nothing selected".
