...
/**
 * Due-date filter row inside the issue layout header filters Popover.
 *
 ...
 * preset's value in / out of the active due-date filter set; submitting a custom range yields a
 * `string[]` that the parent persists.
 *
 * Props (`Props`):
 *   - `appliedFilters` (`string[] | null`, required): currently-selected due-date filter values
 *     for the `target_date` filter slot — either preset keys (from `DATE_AFTER_FILTER_OPTIONS`)
 *     or hyphen-separated custom date-range strings.
